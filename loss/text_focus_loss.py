...
class TextFocusLoss(nn.Module):
    ...
    def build_up_transformer(self):
        ...
        # transformer.load_state_dict(torch.load(self.cfg.transformer_pretrained))
        # transformer.eval()
        transformer.train()
    ...
    def pred_decoder(self, pred):
        batch = len(pred)

        length = [len(i) for i in pred]
        ...
        for i in range(batch):
            ...
            for j in range(length[i] - 1):
                # input_tensor[i][j + 1] = self.english_dict[label[i][j]]
                string += list(self.english_dict.keys())[list(self.english_dict.values()).index(int(max(pred_exp[i][j])))]
    ...
    def forward(self,sr_img, hr_img, label):
        ...
        hr_pred, word_attention_map_gt, hr_correct_list, word_decoder_result = self.transformer(to_gray_tensor(hr_img), length_tensor,
                                                                        input_tensor, test=False)
        sr_pred, word_attention_map_pred, sr_correct_list, word_decoder_result = self.transformer(to_gray_tensor(sr_img), length_tensor,
                                                                        input_tensor, test=False)
        ...
        # decode_strings = self.pred_decoder(sr_pred)
        loss = mse_loss + attention_loss * 10 + recognition_loss * 0.0005
        return loss, mse_loss, attention_loss, recognition_loss, word_decoder_result
        # else:
        #     attention_loss = -1
        #     recognition_loss = -1
        #     loss = mse_loss
